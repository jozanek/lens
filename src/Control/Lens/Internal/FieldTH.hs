  ...
  do keywords <- lift activeKeywords
     ...
     case _classyLenses rules tyName of
       Just (className, methodName) ->
         do lift $ checkClassyName keywords tyName methodName
...
-- | Fail early, in 'Q', when a 'FieldNamer' generates a definition or method
-- name that is reserved in the splice's language. Without this check GHC
-- rejects the generated declarations with an "Illegal variable name" error that
-- names neither the field nor the data type responsible. See issue #762.
checkDefName :: Set String -> Name -> (DefName, Maybe Name) -> Q ()
checkDefName keywords tyName (defName, field) =
  when (nameBase n `Set.member` keywords) $ fail $ unlines
    [ "Cannot generate an optic named '" ++ nameBase n ++ "' for the field"
      ++ fieldMsg ++ " of '" ++ nameBase tyName ++ "':"
    , "'" ++ nameBase n ++ "' is a Haskell keyword or is reserved by an enabled extension."
    , "Possible fixes: rename the field, or use a 'lensField' naming rule that"
    , "avoids reserved names, e.g. by wrapping the namer with 'avoidKeywordsNamer'"
    , "or 'avoidNamesNamer (haskellKeywords <> ghcExtensionKeywords)'."
    ]
  where
  n = case defName of
        TopName x      -> x
        MethodName _ x -> x
  fieldMsg = maybe "" (\f -> " '" ++ nameBase f ++ "'") field

-- | As 'checkDefName', but for the class method generated by a 'ClassyNamer'
-- (e.g. 'makeClassy' on a type named @Where@).
checkClassyName :: Set String -> Name -> Name -> Q ()
checkClassyName keywords tyName methodName =
  when (nameBase methodName `Set.member` keywords) $ fail $ unlines
    [ "Cannot generate a classy lens method named '" ++ nameBase methodName
      ++ "' for '" ++ nameBase tyName ++ "':"
    , "'" ++ nameBase methodName ++ "' is a Haskell keyword or is reserved by an enabled extension."
    , "Possible fixes: rename the data type, or use a 'lensClass' naming rule"
    , "that avoids reserved names, e.g. by wrapping the namer with"
    , "'avoidKeywordsClassyNamer' or"
    , "'avoidNamesClassyNamer (haskellKeywords <> ghcExtensionKeywords)'."
    ]
